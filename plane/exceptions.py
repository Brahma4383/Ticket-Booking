"""
Errors the plane endpoints raise, and the handler that turns them into JSON.
...
class PlaneAPIError(APIException):
    """Base for the plane module's own failures."""

    status_code = status.HTTP_400_BAD_REQUEST
    default_code = 'plane_error'
...
class FlightNotFound(PlaneAPIError):
    status_code = status.HTTP_404_NOT_FOUND
    default_code = 'flight_not_found'
    default_detail = 'That flight is no longer available.'


class BookingNotFound(PlaneAPIError):
    status_code = status.HTTP_404_NOT_FOUND
    default_code = 'booking_not_found'
    default_detail = 'No booking was found for that reference.'


class InvalidSelection(PlaneAPIError):
    """A fare brand, seat, add-on or date that does not fit the flight."""

    default_code = 'invalid_selection'
    default_detail = 'That selection is not valid for this flight.'


class SeatUnavailable(PlaneAPIError):
    """
    Someone else took a seat first. 409 rather than 400: the request was well
    formed, the cabin filled up underneath it.
    """

    status_code = status.HTTP_409_CONFLICT
    default_code = 'seat_unavailable'
    default_detail = 'One or more of those seats has just been taken.'
